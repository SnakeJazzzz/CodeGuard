...
class TestJobCreation:
    ...
    def test_create_job_with_custom_status(self, db_session):
        """Test creating job with custom status."""
        job_id = "test_job_002"
    ...
    def test_create_multiple_jobs(self, db_session):
        ...
        for i, job in enumerate(jobs):
            assert job['job_id'] == f"test_job_{i:03d}"
...
class TestJobResults:
    ...
    def test_get_results_nonexistent_job(self, db_session):
        ...
        results = get_job_results(db_session, "nonexistent_job")
        assert isinstance(results, list)
        # Should return empty list for nonexistent job
...
class TestJobStatusUpdates:
    ...
    def test_update_job_status(self, db_session):
        ...
        # Verify update by checking job exists and status was updated
        assert success is True
        exists = job_exists(db_session, job_id)
        assert exists is True
    ...
    def test_update_status_through_workflow(self, db_session):
        ...
        success1 = update_job_status(db_session, job_id, 'running')
        assert success1 is True

        success2 = update_job_status(db_session, job_id, 'completed')
...
class TestJobSummary:
    ...
    def test_get_job_summary(self, db_session):
        ...
        summary = get_job_summary(db_session, job_id)

        assert summary is not None
        assert 'job_id' in summary
        assert 'total_comparisons' in summary or 'pair_count' in summary

    def test_get_summary_nonexistent_job(self, db_session):
        """Test getting summary for nonexistent job."""
        summary = get_job_summary(db_session, "nonexistent_job")
...
class TestConcurrentOperations:
    ...
    def test_update_and_check_immediately(self, db_session):
        ...
        # Update
        success = update_job_status(db_session, job_id, 'completed')
