...
def test_file_upload_to_database_flow(test_database, test_files):
    ...
    assert (
        abs(retrieved_results[0]["token_similarity"] - comparison_data["token_similarity"]) < 0.001
    )
    assert (
        abs(retrieved_results[0]["confidence_score"] - comparison_data["confidence_score"]) < 0.001
    )
    ...
    for i in range(len(test_files)):
        for j in range(i + 1, len(test_files)):
            ...
            if file1.stat().st_size == 0 or file2.stat().st_size == 0:
                ...
                result = {"similarity_score": 0.0, "is_plagiarism": False}
            else:
                try:
